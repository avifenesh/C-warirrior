...
- Web build targets the Axum API endpoints listed in `docs/architecture/system.md` (HTTP contract table). WASM provides local projection for faster input/render.
- Keep the HTTP/WASM flows synchronized with `docs/logic-mindmap.md`, which maps each route/handler to its Rust and Svelte sources.
